...
class Ueditor extends Controller{
	public function index(){
        ...
        $CONFIG = json_decode(preg_replace("/\/\*[\s\S]+?\*\//", "", file_get_contents("./static/admin/plugins/ueditor/php/config.json")), true);
        $action = $_GET['action'];
	...
	private function upFile($fieldName){
		...
		if($info){
			...
			if($isImg){
				...
				}else if($water ==2 ){//图片水印
				    $image->water('./static/admin/img/logo.png',3,100)->save('.'.$fname);
				}
